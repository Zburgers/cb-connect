...
| D-013 | Prediction benchmark dataset authority, consent basis, calibration split and preregistration approver | Product, privacy and engineering/statistical reviewer | Real Gate 3 benchmark outcome viewing/promotion | Product protocol frozen 2026-09-20; dataset permission/consent basis and named statistical/preregistration approval still required before real outcomes | G3-BENCH-V1 freezes a 60/20/20 user-level development/calibration/locked-evaluation split, walk-forward evaluation, external-data Level-2 use, metrics/subgroups and promotion criteria; criteria may not be loosened after results |
...
## D-013 — Gate 3 benchmark protocol freeze

- Decision: Freeze G3-BENCH-V1 before any real candidate outcome review. Users are deterministically partitioned by pseudonymous stable hash into 60% development, 20% calibration and 20% locked final evaluation; each user's folds remain chronological. External academic data may be used for reproduction, estimator selection, robustness/subgroup work and initial history-band calibration only when its source/license or permission, cohort, de-identification, fields, checksum and transfer limitations are recorded. It does not become a Gate 3 population-trained point-prediction prior. Point predictions remain configured-baseline plus the user's own eligible history. Metrics, subgroups, calibration targets and promotion criteria are those frozen in `docs/research/cycle-benchmark-protocol.md`.
- Alternatives considered: synthetic-only qualification was judged too weak for eventual model promotion; importing an external population-trained model directly into Gate 3 was rejected because cohort transferability is not established; using the same data for model selection and final evaluation was rejected for leakage/overfitting risk.
- Approver and authority: Product/model policy approved by the sole project owner on 2026-09-20. Dataset permission/consent basis where applicable and the named engineering/statistical preregistration review authority remain required before real external/CB Connect outcome reports are opened.
- Approved on: Product protocol 2026-09-20; remaining authority pending.
- Applies from commit/deployment: Planning/implementation branch `codex/gate3implementation`; synthetic/golden benchmark implementation may proceed. No production exposure is authorized.
- Evidence or runbook: `docs/research/cycle-benchmark-protocol.md`, `docs/research/2026-09-20-gate-3-prediction-literature-review.md`, and `docs/plans/2026-09-20-gate-3-personalized-prediction-implementation.md`.
- Review/expiry date: Revalidate before the first real dataset manifest is admitted and before opening every new locked evaluation holdout.

## D-017 — Gate 3 prediction product/model contract

- Decision: Approve the complete Gate 3 design freeze: provisional three-interval personalization floor subject to stricter benchmark evidence; one globally approved estimator first; whole eligible history retained with rolling/recency candidates; isolated surprises primarily increase uncertainty while persistent shifts may adapt the point; high variability still receives the best defensible range with lower quality; possible missing logs are preserved/flagged, never fabricated into cycles; authorized partner-assisted exact starts are immediately eligible under D-009; narrow partner correction is allowed only for authorized assisted records while primary correction/deletion remains final; hard context segmentation is private and user-controlled while automatic change detection may only influence/suggest; user-facing probability is reserved for calibrated intervals while an internal numeric quality diagnostic may remain engineering-only; subjective feedback is not a modelling label; Gate 3 prediction features remain start-to-start only; partner projection is reduced and care-oriented; evaluation uses development/calibration/locked holdout partitions.
- Alternatives considered: primary-confirmation gating for partner entries, automatic hard segmentation, per-user estimator routing, public-data population point priors, arbitrary user-visible confidence percentages, symptom/pain features, and variability-driven abstention were rejected for Gate 3.
- Approver and authority: Sole project owner, approved directly in the 2026-09-20 Gate 3 design review. D-012/D-013/D-015/D-016 retain their separate authorities and blockers.

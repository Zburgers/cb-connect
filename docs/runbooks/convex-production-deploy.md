...
- Compatibility: `v1`
- Workflow consistency variable: `CB_CONNECT_PRODUCTION_DEPLOYMENT`
...
The workflow rejects any consistency value other than the approved selector
and invokes all stateful Convex operations through
`scripts/convex-safe-exec` in explicit `production` mode. The wrapper derives
the target from the deployment prefix of `CONVEX_DEPLOY_KEY`, requires that
prefix to equal the approved selector, rejects `CONVEX_DEPLOYMENT` in its
execution context, and verifies effective backend identity immediately before
and after each operation. No CLI default or `--prod` shortcut is valid
...
whose non-secret target prefix is `dev:hallowed-hummingbird-284`, provided
through `CONVEX_DEPLOY_KEY`, and invoke the wrapper in explicit `test` mode.
Do not set `CONVEX_DEPLOYMENT` in that executor context. Verify
`getBackendIdentity`, the `v1`

...
- Timestamp: 2026-08-12T23:39:48+05:30
- Agent/session: Codex primary agent; session identifier unavailable
- Task and plan IDs: Gate 0 E2/E3 fixture registration and teardown remediation; PR #17 qualification
- Starting commit: `db41725`
- Ending commit: `db41725` with this remediation uncommitted before the next focused commit
- Work performed: Observed PR run `31623624502` pass deterministic qualification and fail at `primary-register` with the redacted `fixture_cleanup_identity_mismatch` reason. Reproduced the same boundary on the approved isolated Clerk/Convex development pair. Added regressions for the empty-email partner transition and pre-registration cleanup recovery. Preserved exact Clerk subject, role, durable-run and couple membership checks while permitting only an empty JWT-derived email for those exact run-owned identities. Made teardown mint a fresh Clerk test sign-in for the deterministic primary, obtain the Convex token before navigation, close the app before cleanup to prevent reactive `ensureUser` recreation, report only redacted substages, and prove zero residue. Changed CI to run desktop and mobile as separate fixture lifecycles with project-scoped run IDs so one Clerk storage state is never replayed across projects.
- Files changed: `.github/workflows/ci.yml`; `convex/mutations/fixtureCleanup.ts`; `convex/mutations/fixtureCleanup.test.ts`; `e2e/auth.global.teardown.ts`; `scripts/tests/release-smoke-workflow.test.sh`; `docs/evidence/reliability-gate-0/e2-live-proof.md`; `docs/evidence/reliability-gate-0/e3-live-proof.md`; `docs/evidence/reliability-gate-0/REPORT.md`; this append-only log
- Commands and outcomes: Focused fixture cleanup regression failed before implementation and passed 8/8 after it. `npx convex dev --once` published only to `dev:hallowed-hummingbird-284`. Fresh desktop and mobile release-smoke invocations each passed 1/1 with zero skips, independent Clerk/Convex fixture runs and teardown `remaining=false`; failed diagnostic fixtures were subsequently cascaded to zero and their synthetic Clerk users deleted. Final `npm run build` passed Next.js 15.5.22 with 13/13 static pages; typecheck passed; full unit tests passed 18 files/101 tests; every shell policy test passed; production audit reported zero vulnerabilities; official actionlint 1.7.12 and `git diff --check` passed.
- Convex deployment class and status: isolated `dev:hallowed-hummingbird-284` only; reviewed fixture functions were published and synthetic E2/E3 data was created and removed there. No production Convex deployment, production data access, PM2 operation, promotion or restore occurred.
...
- Timestamp: 2026-08-13T10:55:50+05:30
- Agent/session: Codex primary agent; session identifier unavailable
- Task and plan IDs: Gate 0 ship-hardening and technical-debt closeout; PR #17
- Starting commit: `de8af6a`
- Ending commit: `c39ddcc` with this append-only evidence entry uncommitted
- Work performed: Audited the planned `/api/health` and `/api/ready` contracts and confirmed they are intentionally distinct: health is process liveness, while ready is bounded dependency and compatibility readiness. Added that decision to `issues.md`. Excluded both operational endpoints from Clerk middleware so incomplete Clerk configuration cannot turn liveness/readiness into middleware 500s. Isolated authenticated release smoke in `playwright.release.config.ts`, restored a normal default Playwright config, restricted browser discovery to `*.spec.ts`, and added a packaged standalone runtime smoke to CI. Recorded the remaining technical debt: repeated release selectors, public-but-fail-closed fixture cleanup functions, large regex-driven workflow policy, and broad dependency ranges.
- Files changed: `.github/workflows/ci.yml`; `DEPLOYMENT.md`; `docs/evidence/reliability-gate-0/REPORT.md`; `docs/evidence/reliability-gate-0/ship-hardening-2026-08-13.md`; `issues.md`; `middleware.ts`; `middleware.test.ts`; `package.json`; `playwright.config.ts`; `playwright.release.config.ts`; `scripts/tests/ci-workflow.test.sh`; `scripts/tests/release-smoke-workflow.test.sh`; `scripts/tests/standalone-runtime.test.sh`; `vitest.config.ts`; this append-only log
- Commands and outcomes: `git diff --check` passed; all CI/deploy/PM2/release-smoke/verify-release/auth-fixture/package/rollback policy tests passed; `npm run test:unit -- --run` passed 19 files/103 tests; `npm run typecheck` passed; `npm run build` passed; default and release Playwright listing passed with the intended project boundaries; packaged standalone health/readiness smoke passed; `npm audit --omit=dev` reported 0 vulnerabilities. No production selector, credential, user identifier or health data was recorded.
- Convex deployment class and status: none; no Convex publish, production access, production mutation, PM2 operation, promotion or restore occurred.
